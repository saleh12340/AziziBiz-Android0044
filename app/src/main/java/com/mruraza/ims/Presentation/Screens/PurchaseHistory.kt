...
import com.mruraza.ims.Presentation.ViewModel.PurchaseHistoryViewModel
...
fun purchaseHistory(
    ...
    Scaffold(
        ...
        val allBillsState by purchaseHistoryViewModel.allBills.collectAsStateWithLifecycle()
        var isDetailsOpen by remember { mutableStateOf(false) }
        var selectedBill by remember { mutableStateOf<PurchaseBill?>(null) }
        when (allBillsState) {
            ...
            is Resources.Success -> {
                var allBills = (allBillsState as Resources.Success).data
...
@Composable
private fun historyLayoutPurchase(
    list: List<PurchaseBill>,
    modifier: Modifier,
    onItemSelect: (PurchaseBill) -> Unit
) {
...
private fun BillDetailedLayout(
    ...
    Column(
        ...
        Box(modifier = Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
            Text(text = "${bill.supplier.name} Bill", style = TextStyle(fontSize = 18.sp))
            IconButton(
                onClick = {
                    onClose()
                },
...
private fun getCleanedBill(bill: List<PurchaseBill>): List<PurchaseBill> {
    val cleanedList = mutableListOf<PurchaseBill>()
    bill.forEach { it ->
